 ...
 * @brief  FDCAN1 motor control task implementation.
 * @version 0.1
 * @date 2025-11-18
 *
 * @note   This file implements the FreeRTOS task for controlling motors on
 *         FDCAN1 bus. It includes motor initialization, enabling with retry
 *         mechanism, and periodic control command sending.
 *
 * Zhengbi Yong
 *
 */
...
/**
 * @brief  Control loop period (milliseconds).
 *
 * @note   This variable controls the period of the motor control loop.
 ...
 *         2. Initialize FDCAN1 bus and enable all motors
 ...
 *            - Delay CHASSR_TIME milliseconds (default 1ms)
 *
 * @note   Control frequency: 1kHz (when CHASSR_TIME = 1ms)
...
void fdcan1_task_(void) {
  ...
  /* Step 2: Initialize FDCAN1 bus and enable all motors */
  ...
  /* Set start flag (already set in fdcan1_init, but ensure it's set) */
 ...
 *
 * @param  bus Pointer to FDCAN1 bus structure (must not be NULL).
 *
 * @retval None
 *
